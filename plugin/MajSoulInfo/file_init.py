...
if not os.path.exists(r"./config/MajSoulInfo/config.yml"):
    ...
    cfg = dict(qhpt=True, qhinfo=True, qhsl=True, qhyb=True, qhpaipu=True, broadcast='image', query_limit=10, linknum=1,
               link_update=True, ignore_history=True, link_freshtime='2:33', autoquery=True, dailydrawcount=3,
               echarts=True, disptgroup=[0], authenticationgroup=[0], disinfogroup=[0], disslgroup=[0], disybgroup=[0],
...
if not os.path.exists(r"./config/MajSoulInfo/command.yml"):
    ...
    _cmd = {
        ...
        'qhinfo': r'(qhinfo|雀魂玩家详情)\s*(\S+)\s*([34])?\s*(\w+)?\s*(\w+)?\s*$',
